...
exports.client = function (config) {
  ...
  instance.available = function() {
    ...
    sock.on('connect', function() {
      logger.info('connected to '+mpdConf.port);
      deferred.resolve();
    });

    sock.on('error', function(error){
      //logger.error('error for '+mpdConf.port, error);

      setTimeout(function(){
        sock.connect(mpdConf.port);
      }, 10);
    });
  ...
  instance.ready = function() {
    mpdConf = mpdConfig.new();
    return mpdConf.write(config).then(this.spawn)
