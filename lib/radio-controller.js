  ...
  function respondToCommand(data) {
    ...
    if(commandAction) {
      commandAction(radio, command).then(function() {
        ...
        console.log("Replying to ", replyToQueue);
        messagingClient.sendToQueue(replyToQueue, {correlationId: data.correlationId, error: false});
      }, utils.failedPromiseHandler());
    } else {
      var error = new Error("Don't know what to do with" + command.action);
      logger.warn(error, command);
      data.ack();
      messagingClient.sendToQueue(replyToQueue, {correlationId: data.correlationId, error: error});
    }
  }
}
